...
const previewEncryption = async (req, res) => {
    try {
        const { text, algorithm } = req.body;
        if (!text) return res.status(400).json({ encryptedText: '' });
        
        // Use a static demo key
        const key = crypto.createHash('sha256').update('demo_key').digest();
